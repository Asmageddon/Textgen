...
s+=["I'm $willing_to to pay a lot for $cont_liquid of $year'' $alcohol"]
s+=["Could you $action_fetch me some $item?"]
s+=["$action_fetch for me a $animal $bodypart from it's $loc_nature"]
...
s+=["I want you to kill $adjective $animal that has been rampaging around the $loc_civ"]
s+=["A $adjective $animal drank a $cont_liquid of my finest $alcohol, $action_kill that $person_addict!"]
...
s+=["$name, that $a_mad $w_creator of $adjective $item must be killed."]
...
s+=["You meet a $a_mystery $person dressed in $color $a_cloth $cloth: '$action yourself, $gqb says"]
...
s+=["There is a $person_any infected with $disease in our $location'!!!"]
s+=["There is no $person_any like our $name'!"]
...
s+=["The $a_good $name was $action_kill'ed by a $a_evil $monster"]
s+=["A $person ordered everybody to train $martial_art"]
s+=["$digit_n0'$digit'$digit tons of $mat_solid were used to build a $a_size $fortification'!"]
s+=["It has been proved that $anything'+$anything'=$anything"]
s+=["By utilizing $a_arcane techniques you can combine $anything with $anything to create the $a_power $cutlery'!"]
s+=["Have faith in $god and you will be $salvation'!"]
s+=["Thou shalt witness my $superpower and tremble in fear, $a_evil $demon"]
s+=["In the name of $element_magic, $outfit $creature shall $action_evil you and deliver $a_death death you deserve!"]
s+=["$monster $relatives will $action $person_who as they did in $year with $anything"]
s+=["spam time! $anything $anything $anything $anything"]
s+=["You need to ask $name the $humanoid for help if you want to take down $name, $a_evil $monster"]
